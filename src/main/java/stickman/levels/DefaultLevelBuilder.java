...
public class DefaultLevelBuilder extends LevelBuilder{
    ...
    public DefaultLevelBuilder(JSONObject configuration, boolean lastLevel) {
    ...
    protected void buildHero() {
        ...
        long hp = (long)hero_obj.get("hp");
        boolean isSmall = ((Boolean)hero_obj.get("small"));
        hero = Hero.getHero();
        // if the hero has not been initialised
        if (!((Hero)hero).isInit()){
            ((Hero) hero).initHero(x, y, HeroPathStateContext.Actor.DEFAULT,hp);
        }
        else{
            ((Hero) hero).resetHeroPos();
        }
        if (isSmall) {
            hero.setWidth(20);
            hero.setHeight(34);
    ...
    @Override
    protected void buildScoreTracker() {
        this.levelExpectFinishTime = (long) configuration.get("finishTime");
    }
}
